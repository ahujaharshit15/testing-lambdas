...
```bash
day2$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
day2$ sam local start-api
day2$ curl http://localhost:3000/
```
...
```bash
day2$ sam logs -n HelloWorldFunction --stack-name day2 --tail
```
...
```bash
aws cloudformation delete-stack --stack-name day2
```
